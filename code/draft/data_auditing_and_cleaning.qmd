...
raw_metadata <- read_xlsx('metada_work_version.xlsx', guess_max = 4000) 
```
...
raw_metadata %>% 
  mutate(name_orig = case_when(grepl('htt', name_orig) & 
                               ...
                               state_province == 'Grand Est' & group == 'Stoneflies' ~ 
                                 'Liste rouge des Ephémères – Trichoptères – Plécoptères du Grand Est',
                               .default = name_orig))%>%
  filter(state_province == 'Grand Est') %>% select(group, name_orig)%>%view()
  
raw_metadata%>%
  mutate(name_orig = ifelse(grepl('https://www.ogbw', link_ref) & 
                         state_province == 'Baden-Württemberg',
                       str_glue('Rote Liste und kommentiertes Verzeichnis der Brutvogelarten Baden-Württembergs'), name_orig)) %>%
  mutate(name_orig = ifelse(grepl('https://www.researchgate.net/publication/260814822_Rote_Liste_Wandernder_Vogelarten', link_ref),
                       str_glue('Rote Liste wandernder Vogelarten Deutschlands'), name_orig)) %>%
  mutate(year = ifelse(grepl('https://www.researchgate.net/publication/260814822_Rote_Liste_Wandernder_Vogelarten', link_ref),
                       str_glue('2012'), name_orig))%>%
  mutate(year = ifelse(grepl('https://www.ogbw', link_ref) & 
                         state_province == 'Baden-Württemberg',
                       str_glue('2013'), year)) %>%view()
